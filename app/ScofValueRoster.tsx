...
export default function ScofValueRoster({ scene }: { scene: FinaleRosterId }) {
  ...
  return (
    <div className={`scofValueRoster roster-${scene}`} aria-live="polite" aria-atomic="true">
      ...
      {scene === "horizon" && <div className="rosterMonument horizonMonument"><small>Long-horizon aspiration · Per SCOF</small><b>{formatEur(scofValueConfig.horizon.eur)}</b><strong className="horizonKesEquivalent">≈ {horizonKes}</strong><em>Illustrative conversion at KSh {scofValueConfig.fxReference.eurKes.toFixed(2)} per EUR</em><span>Aspiration only · No guaranteed market outcome</span></div>}
