...
test("keeps required experience, governance and mobile safeguards in source", async () => {
  ...
  assert.match(page, /className="patronCard patronMediaCard"[\s\S]*?className="governanceColumn"[\s\S]*?className="governanceFlow"[\s\S]*?className="patronProfilePanel"/);
  assert.match(page, /className="patronProfilePanel" aria-labelledby="patron-profile-title"/);
  assert.match(page, /className="patronProfileNotices"/);
  assert.match(page, /className="operationsNotice" aria-labelledby="solomon-ops-title"/);
  assert.match(page, /className="operationsTusker" aria-hidden="true">🐘/);
  assert.match(page, /className="operationsBoundary"[\s\S]*?🗳️[\s\S]*?2027 campaign boundary/);
  assert.match(page, /<h3 id="solomon-ops-title">Solomon Ops remains separate\.<\/h3>/);
  ...
  assert.match(css, /\.governanceColumn\{[^}]*display:flex/);
  assert.match(css, /\.patronProfilePanel\{/);
  assert.match(css, /\.patronProfileNotices\{[^}]*grid-template-columns:minmax\(0,\.82fr\) minmax\(0,1\.18fr\)/);
  ...
  assert.match(css, /@media\(max-width:1080px\)\{[\s\S]*?\.patronProfilePanel\{order:-1\}/);
  assert.match(css, /@media\(max-width:620px\)\{[\s\S]*?\.patronProfileNotices\{grid-template-columns:1fr\}/);
  ...
  assert.match(css, /Page rhythm V1: compact section handoffs without shrinking real content/);
  assert.match(css, /--section-space-start:clamp\(44px,4\.2vw,64px\)/);
  assert.match(css, /--section-space-end:clamp\(26px,2\.6vw,38px\)/);
  assert.match(css, /\.section\{padding-block:var\(--section-space-start\) var\(--section-space-end\)\}/);
  assert.match(css, /\.sectionHeading\{margin-bottom:var\(--section-heading-gap\)\}/);
  assert.match(css, /\.journeySection blockquote\{margin-top:clamp\(32px,3vw,40px\)\}/);
  assert.match(css, /\.governanceSection\{padding-bottom:var\(--section-compact-end\)\}/);
  assert.match(css, /\.sustainabilitySection\{padding-top:var\(--section-compact-start\)\}/);
  assert.match(css, /\.footerCeremonyHome\{min-height:0\}/);
  assert.match(css, /\.operationsTusker\{/);
  assert.match(css, /\.operationsNotice h3\{/);
  assert.match(css, /@keyframes operationsTuskerFloat/);
  assert.match(css, /@media\(max-width:620px\)\{[\s\S]*?--section-space-start:40px/);
